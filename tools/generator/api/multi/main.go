...
func main() {
	...
	if err := filepath.Walk(yamls, func(path string, info os.FileInfo, err error) error {
		...
		if !info.IsDir() && strings.HasSuffix(info.Name(), ".yaml") {
			...
			// setter
			if err := NewSetter().generate(path, "../../../../api/multi/presentation/server"); err != nil {
				log.Printf("failed to NewSetter().generate: %s", err)
			}

			// handler
			if err := NewHandler().generate(path, "../../../../api/multi/presentation/handler"); err != nil {
