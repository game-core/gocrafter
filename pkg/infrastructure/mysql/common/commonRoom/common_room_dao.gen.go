...
func (s *commonRoomDao) FindOrNilByName(ctx context.Context, name string) (*commonRoom.CommonRoom, error) {
	t := NewCommonRoom()
	res := s.ReadMysqlConn.WithContext(ctx).Where("name = ?", name).Find(t)
	if err := res.Error; err != nil {
		return nil, err
...
func (s *commonRoomDao) FindListByName(ctx context.Context, name string) (commonRoom.CommonRooms, error) {
	ts := NewCommonRooms()
...
func (s *commonRoomDao) Create(ctx context.Context, tx *gorm.DB, m *commonRoom.CommonRoom) (*commonRoom.CommonRoom, error) {
	...
	t := &CommonRoom{
		...
		RoomReleaseType: m.RoomReleaseType,
		Name:            m.Name,
