...
func generateList(structInfo *StructInfo) string {
	return fmt.Sprintf(`List(limit int64, shardKey int) (*%s.%ss, error)`, structInfo.Package, structInfo.Name)
}
...
func generateUpdate(structInfo *StructInfo) string {
	return fmt.Sprintf(
		`Update(entity *%s.%s, shardKey int, tx *gorm.DB) (*%s.%s, error)`,
		structInfo.Package,
		structInfo.Name,
		structInfo.Package,
		structInfo.Name,
...
func generateListByIndex(structInfo *StructInfo, indexFields []string) string {
	...
	return fmt.Sprintf(
		`ListBy%s(%s, shardKey int) (*%s.%ss, error)`,
		strings.Join(indexFields, "And"),
		strings.Join(paramStrings, ","),
		structInfo.Package,
		structInfo.Name,
